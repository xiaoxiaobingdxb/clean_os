...
void init_malloc(bool isKernel) {
    mem_page_size = 4096;  // TODO config
    malloc_state *state;
    if (isKernel) {
        if (!kernel_state) {
            kernel_state =
...
void *_alloc_kernel_mem(size_t size) {
#ifdef KERNEL
    extern uint32_t alloc_kernel_mem(int page_count);
...
void _unalloc_kernel_mem(void *ptr, size_t size) {
#ifdef KERNEL
    extern int unalloc_kernel_mem(uint32_t vaddr, int page_count);
    int page_count = up2(size, mem_page_size) / mem_page_size;
